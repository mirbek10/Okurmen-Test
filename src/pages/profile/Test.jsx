...
export function PracticeSelection() {
  ...
  return (
    <div className="w-full max-w-6xl mx-auto px-4 py-8 md:py-12">
      ...
      {/* Сетка карточек: изменено на 1 колонку по умолчанию */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
        {modes.map((mode) => (
          <div
            key={mode.id}
            onClick={() => navigate(mode.path)}
            className={`group relative bg-white border-2 border-slate-100 ${mode.border} rounded-3xl p-6 md:p-8 transition-all duration-300 cursor-pointer hover:shadow-2xl hover:shadow-slate-200/50 flex flex-col h-full`}
          >
            {/* Иконка и бейдж */}
            ...
            <div className="flex items-center justify-between mt-auto pt-6 border-t border-slate-50">
              ...
              <div className={`${mode.button} text-white p-2.5 md:p-3 rounded-xl transition-transform group-hover:scale-110 active:scale-95 shadow-lg`}>
            ...
            {/* Декоративный эффект */}
            {mode.id === 'real' && (
              <div className="absolute inset-0 border-2 border-amber-400/20 rounded-3xl pointer-events-none animate-pulse" />
            )}
          </div>
        ))}
      </div>

      {/* Инфо-блок: адаптация под мобильные */}
      <div className="mt-8 md:mt-12 p-5 md:p-6 bg-white rounded-3xl border border-slate-100 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-6">
        <div className="flex items-center gap-4 text-center sm:text-left flex-col sm:flex-row">
          <div className="shrink-0 w-10 h-10 bg-indigo-50 rounded-xl flex items-center justify-center text-indigo-600 font-bold">
            ?
          </div>
          <p className="text-sm text-slate-600 font-medium">
            Не уверены в силах? Пройдите <span className="text-indigo-600 font-bold whitespace-nowrap">Frontend тренировку</span>, чтобы разогреться.
          </p>
        </div>
        <button 
          onClick={() => navigate('/user/history')}
          className="w-full sm:w-auto text-indigo-600 text-sm font-bold hover:underline bg-indigo-50 px-6 py-3 rounded-xl transition-colors hover:bg-indigo-100 whitespace-nowrap"
        >
          История результатов →
        </button>
      </div>
